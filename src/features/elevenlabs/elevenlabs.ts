...
export async function synthesizeVoice(message: string, selectedVrm: number) {
  try {
    const response = await axios.post("https://www.waifu-gamma.vercel.app/api/synthesizeVoice", { 
      message,
...
export async function getVoices() {
  try {
    const response = await axios.get("https://www.waifu-gamma.vercel.app/api/getVoices");
